...
from cnntransformer.pingxing_trans import PX_TRANS
from results import Results_level

# from test import my_val_data,my_train_data
from dataset_load import my_val_data,my_train_data
...
# test_dataset,test_loader = my_test_data()
...
# scheduler = CosineAnnealingLR(optimizer, T_max=100, eta_min=lr*0.001)
######+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
def train(model,train_loader,val_loader,criterion ):
    ...
    for i in (np.arange(1, 4) * 5):
        ...
        early_stopping = EarlyStopping(r'C:\Users\WjtDg\Desktop\cnn_transformer\checkpoint')
        for epoch in range(num_epoch):
            ...
            for batch_x,batch_y,padding_mask in progress_bar:
                ...
                # batch_x, targets_a, targets_b, lam = mixup_data(batch_x, batch_y, alpha=0.1)
                ...
                # loss = lam * criterion(outputs, targets_a.long().squeeze(-1)) + (1 - lam) * criterion(outputs, targets_b.long().squeeze(-1))
                ...
                # predicted = torch.argmax(nn.functional.softmax(outputs, dim=1), 1)  # 使用 argmax 得到预测结果

                # _, predicted = torch.max(outputs, 1)  # 使用 argmax 得到预测结果
            ...
            if val_acc > best_val_acc:
                ...
                #######################################
                # 保存模型
                model_path = r'C:\Users\WjtDg\Desktop\cnn_transformer\checkpoint\model\best_model.pth'
...
def calculate_accuracy1(model,criterion, loader):
    ...
    with torch.no_grad():
        for batch_X, batch_y, padding_mask in loader:
            ...
            total_loss += loss.item() * len(batch_X)
            # _, predicted = torch.max(output, 1)  # 使用 argmax 得到预测结果
            predicted = torch.argmax(nn.functional.softmax(output,dim=1), 1)  # 使用 argmax 得到预测结果


            # _,predicted = torch.max(nn.functional.softmax(output, dim=1), 1)  # 使用 argmax 得到预测结果
...
if __name__ == '__main__':
    model1 = train(model,train_loader,val_loader,criterion )

    print('Validation !!')
    test(model1,val_loader,criterion)

    # xval = np.load(r'C:\Users\WjtDg\Desktop\yanzhong\xval.npy')
    # yval = np.load(r'C:\Users\WjtDg\Desktop\yanzhong\yval.npy')
    # count= np.load(r'C:\Users\WjtDg\Desktop\yanzhong\count_val.npy')
    # print(yval)
    # val_results.validate_patient(model1, torch.tensor(xval), torch.tensor(yval), count)
